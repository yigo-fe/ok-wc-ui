...
import { classMap, defineComponent, html, onMounted, onUpdated } from 'ok-lit'
...
defineComponent(
  ...
  (props, context) => {
    onMounted(() => {
      const tip = createPopper(
        context.$refs.personCell,
        context.$refs.tooltip,
        {
          placement: 'right',
        }
      )
    ...
    onUpdated(() => {
      console.log(context.$refs, 'onUpdated')
    })
    ...
    return () =>
      html`
        ...
        <span ref="personCell" class="ok-person-cell">
          <img src="${props.persons[0].src}" />
        </span>
        <div ref="tooltip" class="tooltip">
          <ok-person-detail
            person=${JSON.stringify(props.persons[0])}
          ></ok-person-detail>
        </div>
      `
  }
