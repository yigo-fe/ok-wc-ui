	...
	<ul ref="okUploadImage" class="ok-upload-list-image">
		...
		>
					...
					>
						{{
							item.status === 'uploading'
								? parseInt((item.percentage || 0).toFixed(), 10) + '%'
								: uploadFailText
						}}</span
					>
					<ok-progress
			...
			<img
				...
				:class="{
					'ok-upload-list__item-thumbnail': true,
					'ok-upload-list__item-default-thumb': isDefaultThumb(item),
				}"
				:src="
					!isDefaultThumb(item) ? item.response.data[0].thumb_url : defaultThumb
				"
			/>
			...
			<span v-if="item.status === 'fail'" class="ok-upload-list__item-actions">
				...
				<!--删除-->
				<i class="operation-icon" @click="handleRemove(item)">
					<svg
						t="1625035260775"
						viewBox="0 0 1024 1024"
						version="1.1"
			...
			>
				...
				<!--删除-->
				<i v-if="showRemove" class="operation-icon" @click="handleRemove(item)">
					<svg
						t="1625035260775"
						viewBox="0 0 1024 1024"
						version="1.1"
...
import defaultThumb from '../../assets/file-icon/icon_file-image_colorful.svg'
		...
		const isDefaultThumb = (item: any): boolean => {
			return noSupportThumb.includes(
				item.response.data[0].file_name.substring(
					item.response.data[0].file_name.lastIndexOf('.') + 1
				)
			)
		}
