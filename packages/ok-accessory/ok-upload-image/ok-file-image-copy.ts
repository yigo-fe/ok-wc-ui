...
defineComponent(
  ...
  (props, context) => {
      ...
      const options = {
        setup() {
          ...
          const fileList = computed(() => {
            return props.fileList.map((v: any, index: number) => {
              v.last = (index + 1) % rowNumber.value === 0 ? 'last' : ''
              console.log(rowNumber.value, '---', v.last)
              return v
            })
          })
        ...
        template: `
          <li v-for="(file, index) in fileList" :key="file.id" :class="['ok-upload-list__item', 'is-' + file.status, file.last ]" :style="thumbStyle">
    ...
    return () => html`
      ...
      <ul ref="okUploadImage" class="ok-upload-list-image"></ul>
    `
  }
)
