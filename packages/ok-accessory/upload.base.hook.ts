 ...
 * @LastEditors: seadon
 * @LastEditTime: 2021-12-09 13:39:30
...
export default function (props: any, context: any, config: any) {
	...
	const reupload = (rawFile: OkFile) => {
		...
		updateStatus({ status: 'uploading' }, rawFile)
		// 调接口， 上传
		post(Array.isArray(rawFile) ? rawFile[0] : rawFile)
	}
	...
	const handleDetele = (data: any) => {
		let idx = fileLists.value.findIndex(
			(v: any) => v.uid === data.detail?.[0]?.uid
		)
		if (idx === -1) {
			// console.warn('文件不存在')
			return
	...
	const propsValEqulValue = () => {
		...
		if (l1 === l2) {
			...
			same = l1
				? propsValue.value.every((v: any) => fileIds.indexOf(v) > -1)
				: true
		}
		return same
	}
